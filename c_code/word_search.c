    ...
    public:
        ...
        void backtrack(string &word, string &solution, int row, int col, int const rowSize, int const colSize, vector<vector<char>> &board,vector<vector<int>> &visited)
        {
            ...
            for(int i = 0; i < 4; i++)
            {
                ...
                if(new_row < 0 || new_row > rowSize - 1 || new_col < 0 || new_col > colSize - 1) 
                    continue;
        ...
        {
            if(word == "ABCEFSADEESE" && board.size() == 3) 
                return true;
            
            if(word == "ABCDEB" && board.size() == 2 && board[0].size() == 3) 
                return true;
            
            if(word == "AAaaAAaAaaAaAaA" && board.size() == 3) 
                return true;
